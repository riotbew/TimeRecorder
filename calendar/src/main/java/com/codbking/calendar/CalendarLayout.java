...
//        mViewDragHelper.shouldInterceptTouchEvent(ev);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(ydiff) > 5 && Math.abs(ydiff) > Math.abs(xdiff)) {
                    ...
                    if (isClickBtottomView) {
                        boolean isScroll = isScroll(view2,ev);
                        if (!isScroll && mInterceptListener != null) {
                            isScroll = mInterceptListener.onIntercept();
                        }
                        if (ydiff > 0) {
                            //向下
                            if (type == TYPE_OPEN) {
    ...
    private boolean isScroll(View view2, MotionEvent ev) {
        if (!isClickView(view2, ev)) {
            return false;
        }
        if (!(view2 instanceof ViewGroup)) {
            return false;
        }
        View subChild = ((ViewGroup) view2).getChildAt(0);
        if (subChild == null) {
            return false;
        }
        if (view2 instanceof ListView) {
            AbsListView list = (AbsListView) view2;
            if (subChild.getTop() != 0) {
                return true;
            } else if (list.getPositionForView(subChild) != 0) {
                    return true;
            }
        }

        if (view2 instanceof RecyclerView) {
            RecyclerView list = (RecyclerView) view2;
            if (subChild.getTop() != 0) {
                return true;
            } else if (list.getChildAdapterPosition(subChild) != 0) {
                return true;
            }
        }
        ...
        for (int i=0; i<childCount; i++) {
            child = ((ViewGroup) view2).getChildAt(i);
            if (child instanceof ViewGroup) {
                if (isScroll(child, ev)) {
                    return true;
                }
            }
        }
